...
export default function CreateRequestForm({ onRequestCreated }) {

  const [categories, setCategories] = React.useState([]);

  const studentId = 1; // this would come from auth context
  const studentName = 'John Doe'; // this would come from auth context
  ...
  return (
    ...
    >
      {({ isSubmitting }) => (
        <Form className="container mb-2 bg-light p-4 border rounded-3">
          <div>
          ...
          <div className="mt-4 text-center">
            <button type="submit" disabled={isSubmitting} className="btn btn-success">Submit Request</button>
          </div>
        </Form>
      )}
    </Formik>
