import { createSlice } from "@reduxjs/toolkit";

const ticketSlice = createSlice({
  name: "tickets",
  initialState: {
    allTickets: [
      {
        id: 'T101',
        customerName: 'Riya Sharma',
        issueTitle: 'Unable to reset password',
        description: 'Customer is not receiving password reset email.',
        status: 'Open',
        priority: 'High',
        assignedTo: 'Aman',
        starred: false
      },
      {
        id: 'T102',
        customerName: 'Karan Mehta',
        issueTitle: 'Subscription payment failed',
        description: 'Payment gets declined even though card is valid.',
        status: 'In Progress',
        priority: 'Medium',
        assignedTo: 'Sneha',
        starred: true
      },
      {
        id: 'T103',
        customerName: 'Neha Iyer',
        issueTitle: 'App crashes on login',
        description: 'Application closes immediately after entering credentials.',
        status: 'Open',
        priority: 'High',
        assignedTo: 'Rahul',
        starred: false
      },
      {
        id: 'T104',
        customerName: 'Arjun Rao',
        issueTitle: 'Unable to download invoice',
        description: 'Invoice download button is not responding.',
        status: 'Resolved',
        priority: 'Low',
        assignedTo: 'Megha',
        starred: false
      },
      {
        id: 'T105',
        customerName: 'Pooja Nair',
        issueTitle: 'Profile update not saving',
        description: 'Customer profile changes disappear after refresh.',
        status: 'In Progress',
        priority: 'Medium',
        assignedTo: 'Aman',
        starred: true
      }
    ],
    selectedTicketId: null,
    activeFilter: 'All'
  },
  reducers: {
    selectTicket: (state, action) => {
      state.selectedTicketId = action.payload;
    },
    setActiveFilter: (state, action) => {
      state.activeFilter = action.payload;
    },
    updateTicketStatus: (state, action) => {
      state.allTickets = state.allTickets.map((ticket) => (ticket.id === action.payload.id)
       ? {...ticket, status: action.payload.status} : ticket);
    },
    updatePriority: (state, action) => {
      state.allTickets = state.allTickets.map((ticket) => (ticket.id === action.payload.id)
       ? {...ticket, priority: action.payload.priority} : ticket);
    },
    starOrUnstarTicket: (state, action) => {
      state.allTickets = state.allTickets.map((ticket) => (ticket.id === action.payload)
       ? {...ticket, starred: !ticket.starred} : ticket);
    }
  }
});

export const { selectTicket, setActiveFilter, updateTicketStatus, updatePriority, starOrUnstarTicket } = ticketSlice.actions;
export default ticketSlice.reducer;
